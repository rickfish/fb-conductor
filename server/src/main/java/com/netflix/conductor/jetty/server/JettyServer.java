...
public class JettyServer implements Lifecycle {
    ...
    public JettyServer(int port, boolean join, int maxHeaderSize) {
        this.port = port;
        this.join = join;
    ...
    public synchronized void start() throws Exception {
		...
		if(this.maxHeaderSize > 0) {
			logger.info("Jetty max header size will be set to " + this.maxHeaderSize + " bytes");
			for (Connector c : server.getConnectors()) {
		        c.getConnectionFactory(HttpConnectionFactory.class).getHttpConfiguration().setRequestHeaderSize(this.maxHeaderSize);
		    }			
		}
        server.start();
