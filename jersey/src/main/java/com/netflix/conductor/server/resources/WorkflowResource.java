...
public class WorkflowResource {
	...
	private static final String postWorkFlow = "POST_workflow";
    
	private static final String postWorkflowByName = "POST_workflowByName";
    
	private static final String postWorkflowNameCorrelated = "POST_workflowNameCorrelated";
    
	private static final String getSearchWorkflow = "GET_searchWorkflow";
    ...
    public String startWorkflow(StartWorkflowRequest request) {
    	try
    	{
    		...
    		Monitors.recordEndPointTiming(this.getClass().getSimpleName(), postWorkFlow, (System.currentTimeMillis()- startTime));
    		return workflowId;		
    	}catch(Exception e){
    		Monitors.recordEndPointFailure(this.getClass().getSimpleName(), postWorkFlow, e.getStackTrace()[0].getFileName(),
    ...
    public String startWorkflow(@PathParam("name") String name,
		...
		try {
			...
			Monitors.recordEndPointTiming(this.getClass().getSimpleName(), postWorkflowByName, (System.currentTimeMillis() - startTime));
			return workflowId;
		} catch (Exception e) {
			Monitors.recordEndPointFailure(this.getClass().getSimpleName(), postWorkflowByName, e.getStackTrace()[0].getFileName(),
					e.getStackTrace()[0].getLineNumber());
			throw e;
		}
    }
    ...
    public Map<String, List<Workflow>> getWorkflows(@PathParam("name") String name,
    	...
    	try {
    		long startTime = System.currentTimeMillis();
    		Map<String, List<Workflow>> workflowInfo =  workflowService.getWorkflows(name, includeClosed, includeTasks, correlationIds);
    		Monitors.recordEndPointTiming(this.getClass().getSimpleName(), postWorkflowNameCorrelated, ( System.currentTimeMillis() - startTime ));
    		return workflowInfo;
    	}catch(Exception e) {
    		Monitors.recordEndPointFailure(this.getClass().getSimpleName(), postWorkflowNameCorrelated, e.getStackTrace()[0].getFileName(), 
    				e.getStackTrace()[0].getLineNumber());
    		throw e;
    	}
    }
    ...
    public SearchResult<WorkflowSummary> search(@QueryParam("start") @DefaultValue("0") int start,
    	...
    	{
    		long startTime = System.currentTimeMillis();
    		SearchResult<WorkflowSummary> workflowSummary = workflowService.searchWorkflows(start, size, sort, freeText, query);
    		Monitors.recordEndPointTiming(this.getClass().getSimpleName(), getSearchWorkflow, (System.currentTimeMillis()- startTime));
    		return workflowSummary;
    	}
    		catch(Exception e) {
    			Monitors.recordEndPointFailure(this.getClass().getSimpleName(), getSearchWorkflow, e.getStackTrace()[0].getFileName(), 
    					e.getStackTrace()[0].getLineNumber());
    			throw e;
    		}
     }
