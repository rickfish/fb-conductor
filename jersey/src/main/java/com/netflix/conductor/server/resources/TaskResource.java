...
public class TaskResource {
	...
	private static final String getPollTask = "GET_pollTask";
	
	private static final String getBatchPollTask = "GET_batchPollTask";
	
	private static final String postUpdateTask = "POST_updateTask";
	
	private static final String getSearchTasks = "GET_searchTasks";
	...
	public Task poll(@PathParam("tasktype") String taskType,
		...
		try 
		{
			...
			Monitors.recordEndPointTiming(this.getClass().getSimpleName(), getPollTask, (System.currentTimeMillis() - startTime));
			return task;
		}catch(Exception e){
			Monitors.recordEndPointFailure(this.getClass().getSimpleName(), getPollTask, e.getStackTrace()[0].getFileName(), 
	...
	public List<Task> batchPoll(@PathParam("tasktype") String taskType,
		...
		try {
			long startTime = System.currentTimeMillis();
			List<Task> newTasks = taskService.batchPoll(taskType, workerId, domain, count, timeout);
			Monitors.recordEndPointTiming(this.getClass().getSimpleName(), getBatchPollTask,
					(System.currentTimeMillis() - startTime));
			return newTasks;
		} catch (Exception e) {
			Monitors.recordEndPointFailure(this.getClass().getSimpleName(), getBatchPollTask, e.getStackTrace()[0].getFileName(),
					e.getStackTrace()[0].getLineNumber());
			throw e;
		}
	}
	...
	public String updateTask(TaskResult taskResult) {
		try {
			long startTime = System.currentTimeMillis();
			String updatedTaskResult = taskService.updateTask(taskResult);
			Monitors.recordEndPointTiming(this.getClass().getSimpleName(), postUpdateTask,
					(System.currentTimeMillis() - startTime));
			return updatedTaskResult;
		} catch (Exception e) {
			Monitors.recordEndPointFailure(this.getClass().getSimpleName(), postUpdateTask, e.getStackTrace()[0].getFileName(),
					e.getStackTrace()[0].getLineNumber());
			throw e;
		}
	}
	...
	public SearchResult<TaskSummary> search(@QueryParam("start") @DefaultValue("0") int start,
		...
		{
			long startTime = System.currentTimeMillis();
			SearchResult<TaskSummary> taskSummaryResult = taskService.search(start, size, sort, freeText, query);
			Monitors.recordEndPointTiming(this.getClass().getSimpleName(), getSearchTasks, (System.currentTimeMillis() - startTime));
			return taskSummaryResult;
		}
		catch(Exception e)
		{
			Monitors.recordEndPointFailure(this.getClass().getSimpleName(), getSearchTasks, e.getStackTrace()[0].getFileName(), 
					e.getStackTrace()[0].getLineNumber());
			throw e;
		}
	}
