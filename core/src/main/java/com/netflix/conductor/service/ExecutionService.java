...
public class ExecutionService {
	...
	public SearchResult<WorkflowSummary> search(String query, String freeText, int start, int size, List<String> sortOptions) {
		...
		List<WorkflowSummary> workflows = result.getResults().stream().parallel().map(workflowId -> {
			try {
				...
				logger.error("On an ExecutionService.search() for workflow id {}, ExecutionDAOFacade.getWorkflowById() could not find it.", workflowId);
				return null;
			}
	...
	public SearchResult<TaskSummary> searchTasks(String query, String freeText, int start, int size, List<String> sortOptions) {
		...
		List<Task> tasks = executionDAOFacade.getTasksById(result.getResults());

		List<Task> tasksInOriginalOrder = new ArrayList<Task>();
		for(String taskId : result.getResults()) {
			for(Task t : tasks) {
				if(t.getTaskId().equals(taskId)) {
					tasksInOriginalOrder.add(t);
					break;
				}
			}
		}
		
		List<TaskSummary> taskSummaries = tasksInOriginalOrder.stream()
			.parallel()
			.filter(Objects::nonNull)
			.map(task -> {
				try {
		...
		int missing = result.getResults().size() - taskSummaries.size();
		long totalHits = result.getTotalHits() - missing;
		return new SearchResult<>(totalHits, taskSummaries);
	}
