...
public class ExecutionDAOFacade {
    ...
    public void removeWorkflow(String workflowId, boolean archiveWorkflow) {
        try {
            Workflow workflow = getWorkflowById(workflowId, archiveWorkflow);
    ...
    private void removeWorkflowIndex(Workflow workflow , boolean archiveWorkflow) throws JsonProcessingException {
        if (archiveWorkflow) {
            if (workflow.getStatus().isTerminal()) {
            	...
            	if(json != null) {
           			try {
           				JsonElement je = this.jsonParser.parse(new JsonReader(new StringReader(json)));
           				if(je != null) {
           				...
           				if(exceptionMessage != null && exceptionMessage.length() > 200) {
                   			reasonForFailure = String.format("we got this (truncated) exception trying to update ElasticSearch: %s", exceptionMessage.substring(0, 200));
           				} else {
                   			reasonForFailure = String.format("we got this exception trying to update ElasticSearch: %s", exceptionMessage);
           				}
           			}
            	} else {
        			reasonForFailure = "the workflow could not be serialized using ObjectMapper.writeValueAsString";
            	}
            	if(reasonForFailure != null) {
            		LOGGER.error(String.format("Cannot update ElasticSearch with archival info for %s %s because %s, just deleting it from ElasticSearch", 
               				workflow.getWorkflowName(), workflow.getWorkflowId(), reasonForFailure));
    ...
    public List<Task> getTasksById(List<String> taskIds) {
        return executionDAO.getTasks(taskIds);
    }
