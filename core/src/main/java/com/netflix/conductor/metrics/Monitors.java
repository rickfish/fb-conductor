...
public class Monitors {
	...
	public static final String conductorEndPointTiming = "conductor_endpoint_timing";
	
	public static final String conductorEndPointFailure = "conductor_endpoint_failure";
	...
	public static void error(String className, String methodName) {
		...
		emailSender.sendErrorEmail(className, methodName);
	}
	 ...
	 * 
	 */
	// get the first 20 characters of the exception/error message when there is a workflow_server_error
	public static void error(String className, String methodName, String errorMsg) {
		getCounter(className, "workflow_server_error", "methodName", methodName, "error_message", errorMsg.substring(0, 20)).increment();
		emailSender.sendErrorEmail(className, methodName);
	}
	...
	//records the end point failure info for the given end point
	public static void recordEndPointFailure(String className, String methodName, String fileName, int lineNumber) {

		counter(className, conductorEndPointFailure, "method_name", methodName, "file_name", fileName,
				"line_number", "" + lineNumber, "statistic", "count");
		// to keep track of total requests count, increment this counter in both success and failure case.
		recordTotalCount(className, methodName);
	}

	// records the count both for recordEndPointFailure and recordEndPointTiming
	private static void recordTotalCount(String className, String methodName) {
		counter(className, "total_count", "method_name", methodName);
	}
	
	//record the time taken for the given endpoint
	public static void recordEndPointTiming(String className, String methodName, long timeElapsed) {
		
		gauge(className, conductorEndPointTiming, timeElapsed, "method_name", methodName,
				"unit", TimeUnit.MILLISECONDS.name());
		
		counter(className, conductorEndPointTiming, "method_name", methodName, "statistic",
				"count");
