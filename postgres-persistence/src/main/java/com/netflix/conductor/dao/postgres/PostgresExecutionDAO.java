...
public class PostgresExecutionDAO extends PostgresBaseDAO implements ExecutionDAO, RateLimitingDAO, PollDataDAO {
    ...
    public boolean removeWorkflow(String workflowId) {
        ...
        try {
        	...
        	/*
        	 * This exception could just be from trying to get the workflow's tasks. If it is, at least remove the workflow stuff and
        	 * don't worry about the tasks (orphan them).
        	 */
            workflow = getWorkflow(workflowId, false);
        }
        
        if (workflow != null) {
        	final Workflow workflowToRemove = workflow;
            withTransaction(connection -> {
                removeWorkflowDefToWorkflowMapping(connection, workflowToRemove);
                removeWorkflow(connection, workflowId);
                removePendingWorkflow(connection, workflowToRemove.getWorkflowName(), workflowId);
            });
            removed = true;
